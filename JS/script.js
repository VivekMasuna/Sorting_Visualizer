...
function createArray() {
    ...
    for(var i = 0; i < size; i++) {
        ...
        $element.css('height', n.toString() + 'px').text(n);
        $element.css('margin-left', margin_element + 'px');
        $element.css('margin-right', margin_element + 'px');

        // var $heightNumber = $('<span class="height-number">' + n + '</span>');
        // $element.append($heightNumber);
...
function swap(a, b) {
    ...
    // setHeight(a, h2);
    // setHeight(b, h1);
    $("#e" + a).css('height', h2).text(arr[a]);
    $("#e" + b).css('height', h1).text(arr[b]);
}
